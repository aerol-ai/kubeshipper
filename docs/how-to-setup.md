...
If you used `--set auth.token=$(openssl rand -hex 32)` at install time, the token was generated by the shell and never printed to your terminal. Retrieve it with:
...
Or load it into a shell variable for use with `curl`:
...
If you don't know the secret name, list secrets in the release namespace — the one you want ends in `-auth`:

```bash
kubectl get secrets -n kubeshipper
```

The naming pattern is `<helm release name>-auth`. If you used `auth.existingSecret` at install time, the chart did not create a secret — read from the one you supplied (key `auth-token`).

### 4.8 Rotating the auth token
...
# Restart the pod so it picks up the new env from the Secret
kubectl rollout restart deploy/kubeshipper -n kubeshipper
```

The pod must restart to re-read the Secret — env vars from `secretKeyRef` are not hot-reloaded.
