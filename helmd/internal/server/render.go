...
func (s *Server) Render(ctx context.Context, req *pb.RenderRequest) (*pb.RenderResponse, error) {
	...
	crds, scs := scanManifest(rel.Manifest)
	return &pb.RenderResponse{
		Manifest:                 rel.Manifest,
		RequiredCrds:             crds,
		ReferencedStorageClasses: scs,
	}, nil
}

func (s *Server) Preflight(ctx context.Context, req *pb.PreflightRequest) (*pb.PreflightResponse, error) {
	checks := []*pb.PreflightCheck{}
	add := func(name string, blocking, passed bool, msg string) {
		checks = append(checks, &pb.PreflightCheck{
			Name: name, Blocking: blocking, Passed: passed, Message: msg,
		})
	}

	render, err := s.Render(ctx, &pb.RenderRequest{
		Release:    req.Release,
	...
	if err != nil {
		...
		return &pb.PreflightResponse{Ok: false, Checks: checks}, nil
	}
	add("chart-resolvable", true, true, "chart pulled and rendered")

	apiext, _ := apiextclient.NewForConfig(s.restCfg)
	for _, name := range render.RequiredCrds {
		ok := false
		if apiext != nil {
	...
	if hasPVC(render.Manifest) {
		ok, msg := s.defaultStorageClassPresent(ctx)
	...
	for _, host := range scanIngressHosts(render.Manifest) {
	...
	return &pb.PreflightResponse{Ok: ok, Checks: checks}, nil
}

func (s *Server) defaultStorageClassPresent(ctx context.Context) (bool, string) {
	scs, err := s.kc.StorageV1().StorageClasses().List(ctx, metav1.ListOptions{})
	if err != nil {
...
func scanManifest(manifest string) (crds, sclasses []string) {
...
func scanIngressHosts(manifest string) []string {
	...
	for _, d := range splitYAML(manifest) {
		var m map[string]interface{}
